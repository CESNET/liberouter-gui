...
@Component({
	...
	<table>
		...
		<tr *ngFor="let user of users">
			...
			<td>{{ user.role }}</td>
			<td> <button md-raised-button title="Remove user" (click)="removeUser(user)"
				*ngIf="currentUser.user.username != user.username"
			>Delete user</button>
			<button md-raised-button routerLink="{{user.id.$oid}}"
			>View user</button>
			</td>
		</tr>
	</table>
	...
	<a routerLink="add"
		md-raised-button
		routerLinkActive="active-link">Add new user</a>
	`,
	providers : [UsersService]
